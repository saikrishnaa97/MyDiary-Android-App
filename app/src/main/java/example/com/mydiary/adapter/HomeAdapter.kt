 ...
 */
class HomeAdapter(val mIHomeActivityCommunicator: IHomeActivityCommunicator, val data: MutableList<EntryDTO>) : CustomAdapter<HomeAdapter.HomeViewHolder>(){
    ...
    override fun onBindViewHolderImplement(holder: HomeViewHolder?, position: Int) {
        var formatter = SimpleDateFormat("dd/MMM/yyyy")
            ...
            holder?.binding?.clEntryItem?.setOnClickListener{
                mIHomeActivityCommunicator.openFullEntry(data?.get(position).getId())
            }
